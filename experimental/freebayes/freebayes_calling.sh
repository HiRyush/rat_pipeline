#!/bin/bash
# ==============================================================================
# FreeBayes Joint Variant Calling — 15 samples, chromosome-parallel
# ==============================================================================
# bcftools call과 동일한 BAM/region에서 FreeBayes를 실행하여 caller 비교
# ==============================================================================
set -euo pipefail
...
BAM_LIST="/home/yusanghyeon/RAT_project/PHMG_IT/results/joint_calling/bam_list.txt"
...
CHROMS=(chr1 chr2 chr3 chr4 chr5 chr6 chr7 chr8 chr9 chr10 chr11 chr12 chr13 chr14 chr15 chr16 chr17 chr18 chr19 chr20 chrX chrY)

mkdir -p "$OUT_DIR/per_chrom"

# ── Step 1: Chromosome-parallel FreeBayes ────────────────────────────────
echo "================================================================"
echo "Step 1: FreeBayes joint calling (chromosome-parallel)"
echo "================================================================"

# Split callable BED by chromosome
for CHR in "${CHROMS[@]}"; do
    grep -w "^${CHR}" "$CALLABLE" > "$OUT_DIR/per_chrom/${CHR}.bed" 2>/dev/null || true
done
# Remaining scaffolds into one file
grep -v -w -f <(printf '%s\n' "${CHROMS[@]}") "$CALLABLE" > "$OUT_DIR/per_chrom/scaffolds.bed" 2>/dev/null || true

# Build BAM arguments for freebayes
BAM_ARGS=""
while read -r BAM; do
...
run_freebayes() {
    ...
    echo "  $NAME: done ($(grep -cv '^#' "$OUT_VCF" || echo 0) variants)"
}

# Run in parallel batches
RUNNING=0
for CHR in "${CHROMS[@]}" scaffolds; do
    BED="$OUT_DIR/per_chrom/${CHR}.bed"
    VCF="$OUT_DIR/per_chrom/${CHR}_raw.vcf"

    run_freebayes "$BED" "$VCF" "$CHR" &
...
# ── Step 2: Merge chromosome VCFs ────────────────────────────────────────
echo ""
echo "================================================================"
echo "Step 2: Merge chromosome VCFs"
echo "================================================================"

RAW_VCF="$OUT_DIR/freebayes_raw.vcf.gz"
...
# ── Step 3: Filter levels (same as bcftools comparison) ──────────────────
echo ""
echo "================================================================"
echo "Step 3: Filter SNPs at multiple thresholds"
echo "================================================================"
...
# ── Step 4: Per-filter, per-sample evaluation ────────────────────────────
echo ""
echo "================================================================"
echo "Step 4: Per-filter, per-sample evaluation"
echo "================================================================"

SUMMARY="$OUT_DIR/freebayes_evaluation.tsv"
...
echo ""
echo "================================================================"
echo "Step 5: Summary by filter level"
echo "================================================================"
