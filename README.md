...
검출하는 **method/pipeline**의 모든 코드를 raw fastq → 최종 결과 순서대로 step별 정리한 곳.

- **Deliverable은 method 자체** — PHMG_IT / mammary_cancer는 성능 측정용 벤치마크, PHMG_IH는 RNA-only 최종 적용 대상.
...
| `analysis/cross_validation` | mammary transfer 분석 |
...
| `docs/` | method 설계·현황·cross-validation 문서 |

> 파일명 규칙: step 폴더 안에서 `phmg_it__*` / `mammary__*` prefix로 dataset 구분.
> step이 1차축이라 두 dataset의 같은 단계 구현을 나란히 비교할 수 있다 (cross-dataset 일관성이 main claim).

## 데이터셋

| Dataset | 종/ref | 샘플 | DNA truth | 역할 |
|---|---|---|---|---|
| PHMG_IT | Rat / rn7 | 15 (5C+10T) | WGS | 개발 + Validation 1 (PPV 68.4%) |
| mammary_cancer | Rat / rn6 | 23 | WES | Validation 2 (PPV 15.5%, panel/design 의존) |
